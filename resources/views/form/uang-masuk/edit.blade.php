<x-app-layout>
    ...
    <div class="py-6">
        <form action=" {{ url("/masuk/uang-masuk/edit/$uangMasuk->id") }} " method="post">
            @csrf
            <div class="mb-5">
                ...
                <select name="kategori" id="kategori"
                    ...
                    @foreach ($kategoris as $kategori)
                        <option value="{{ $kategori->id }}" @if ($kategori->id == $uangMasuk->kategori_id) selected @endif>
                ...
                <label for="nominal" class="block mt-2 mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    Nominal (Rp)</label>
                <input type="number" id="nominal" name="nominal" value={{ $uangMasuk->nominal }}
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="" required />

                <label for="keterangan" class="block mt-2 mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    Keterangan</label>
                <input type="text" id="keterangan" name="keterangan" value="{{ $uangMasuk->keterangan }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="" required />

                <label for="tanggal" class="block mt-2 mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    tanggal</label>
                <input type="date" id="tanggal" name="tanggal" value="{{ $uangMasuk->tanggal }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="" required />
