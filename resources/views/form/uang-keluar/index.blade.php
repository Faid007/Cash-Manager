                    ...
                    @foreach ($uangKeluars as $uangKeluar)
                        <tr
                            ...
                            <th scope="row"
                                ...
                                {{ $uangKeluar->kategori->name }}
                            </th>
                            <td class="px-6 py-4">
                                Rp. {{ number_format($uangKeluar->nominal, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $uangKeluar->keterangan }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $uangKeluar->tanggal }}
                            </td>
                            ...
                            <div class="flex">
                                    ...
                                    <form action=" {{ url("/keluar/uang-keluar/hapus/$uangKeluar->id") }} "
                                        method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('Apakah Anda yakin ingin menghapus item ini?')"
                                            class="text-white bg-purple-400 focus:ring-4 focus:outline-none focus:ring-purple-400 dark:focus:ring-purple-800 font-medium rounded-lg text-xm px-2 py-1 text-center me-2 mb-2">Hapus</button>
                                    </form>
                                </div>
